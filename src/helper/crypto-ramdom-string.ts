...
const generateForCustomCharactersAsync = async (length: number, characters: string): Promise<any> => {
  // Generating entropy is faster than complex math operations, so we use the simplest way
  const characterCount = characters.length;
  const maxValidSelector = Math.floor(0x10000 / characterCount) * characterCount - 1; // Using values above this will ruin distribution when using modular division
  const entropyLength = 2 * Math.ceil(1.1 * length); // Generating a bit more than required so chances we need more than one pass will be really low
...
const generateRandomBytes = (byteLength: number, type: any, length: number) =>
  crypto.randomBytes(byteLength).toString(type).slice(0, length);

const generateRandomBytesAsync = async (byteLength: number, type: any, length: number) => {
...
const cryptoRandomString = (data?: DataEntity): string => {
